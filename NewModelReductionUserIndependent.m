...
function [net_info, perf] = signfi_cnn_example(csi,label)
    ...
    % Concatenate the first 4 users into a single tensor
    csi_five_users = cat(4, csi1, csi2, csi3, csi4, csi5);

    % Split the labels into training and testing
    csi_abs = abs(csi_five_users);
    csi_ang = angle(csi_five_users);
    ...
    pca_csi_tensor = zeros(200, 5, 3, 7500);
    ...
    layers = [imageInputLayer([M N S]);
              ...
              convolution2dLayer(3, 66,'Padding',3);
              batchNormalizationLayer();
              reluLayer();

              maxPooling2dLayer(3,'Stride',1); 
              convolution2dLayer(2, 66,'Padding',2);
              batchNormalizationLayer();
              reluLayer();
    ...
    trainCsi = cat(4, csi_tensor(:, :, :, 1:split_idx),csi_tensor(:, :, :, 6001:7500));
    testCsi = csi_tensor(:, :, :, split_idx+1:6000);

    trainWord = categorical(cat(1, label(1:split_idx), label(6001:7500)));
    testWord = categorical(label(split_idx+1:6000));
