...
function [net_info, perf] = signfi_cnn_example(csi,label)
    ...
    pca_csi_tensor = zeros(200, 60, 2, 5280);
    ...
    for index = 1:5280 
        ...
        empty_matrix = zeros(200, 60, 2);
            
        for j = 1:60
            csi_array = reshape(csi_matrix(:, j, :), [200, 3]); % The current subcarrier
            ...
            reduced_array = csi_array * coefficient_array(:, 1:2);
            
            reduced_array = reshape(reduced_array, [200, 1, 2]);
